...
$(document).ready(function() {
    ...
    // Requirements check
    $('#check-requirements-btn').on('click', function() {
        const $btn = $(this);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Show next button if all requirements are met
                if (response.success) {
    ...
    $('#check-permissions-btn').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Show next button if all permissions are correct
                if (response.success) {
    ...
    // Database form validation and submission
    $('#database-form').on('submit', function(e) {
        e.preventDefault();
        ...
        $submitBtn.html('<span class="spinner-border spinner-border-sm me-2"></span> Testing Connection...');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    window.location.href = 'install.php?step=5'; // Go to admin step
                } else {
                    $('#database-error')
    ...
    $('#admin-form').on('submit', function(e) {
        ...
        $submitBtn.html('<span class="spinner-border spinner-border-sm me-2"></span> Validating...');
        $submitBtn.prop('disabled', true);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    window.location.href = 'install.php?step=6'; // Go to installation step
                } else {
                    $('#admin-error')
                        .text(response.message)
                        .removeClass('d-none');
    ...
    $('#start-installation-btn').on('click', function() {
        const $btn = $(this);
        const $btnText = $btn.html();
        const $log = $('#installation-log');
        
        $btn.html('<span class="spinner-border spinner-border-sm me-2"></span> Installing...');
        $btn.prop('disabled', true);
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: { action: 'install' },
            timeout: 600000, // 10 minutes timeout for the long-running installation
            success: function(response) {
                if (response.success) {
                    // Display all logs
                ...
                if (status === 'timeout') {
                    ...
                    try {
                        ...
                        if (errorData.message) {
                            ...
                            $('#installation-error').text(errorData.message);
                        }
                    } catch (e) {
                        // Always show the full response regardless of size
    ...
    $('#run-diagnostics-btn').on('click', function() {
        ...
        $btn.html('<span class="spinner-border spinner-border-sm me-2"></span> Running Diagnostics...');
        $btn.prop('disabled', true);
        $('#installation-error').addClass('d-none');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.system_info) {
                    ...
                    $log.append(`Server Software: ${sysInfo.server_software}\n`);
    ...
    $('#create-installer-btn').on('click', function() {
        const $btn = $(this);
        const $btnText = $btn.html();
        const $result = $('#installer-result');
        const $message = $('#installer-message');
        const $download = $('#installer-download');
        
        $btn.html('<span class="spinner-border spinner-border-sm me-2"></span> Creating Installer...');
        $btn.prop('disabled', true);
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: { action: 'create_installer' },
            success: function(response) {
                if (response.success) {
                    $message.html(`<i class="fas fa-check-circle me-2"></i> ${response.message}`);
                    
                    // Show download link
                    $('#download-link').attr('href', response.zipFile);
                    $download.removeClass('d-none');
                    
                    // Redirect to Laravel app with longer delay to ensure server starts
                    $message.append('<br><i class="fas fa-spinner fa-spin me-2"></i> Starting server and redirecting in 5 seconds...');
                    
                    setTimeout(function() {
                        // Try to open the Laravel app
                        const serverUrl = response.serverUrl || 'http://localhost:8000';
                        window.location.href = serverUrl;
                    }, 5000); // Increased to 5 seconds to give server more time to start
                } else {
                    $message.html(`<i class="fas fa-times-circle me-2"></i> ${response.message}`);
            ...
            error: function() {
                $message.html('<i class="fas fa-times-circle me-2"></i> An error occurred while creating the installer package');
                $result.removeClass('d-none');
                $download.addClass('d-none');
                $btn.html($btnText);
                $btn.prop('disabled', false);
